// part missing after addtohistory(inputBuffer);
...
int command_count = 0; /** * Add the most recent command to the history. */

void addtohistory(char inputBuffer[])
{

int i = 0; // add the command to history

strcpy(history[command_count % MAX_COMMANDS], inputBuffer); // add the display-style command to history

while (inputBuffer[i] != '\n' && inputBuffer[i] != '\0')

{

display_history[command_count % MAX_COMMANDS]
...
if (command_count < MAX_COMMANDS) upper = command_count;

else upper = MAX_COMMANDS;
...
printf("Program run by %s ", pw->pw_name);
...
break; /* perror is a library routine that displays a system error message, according to the value of the system variable "errno" which will be set during a function (like fork) that was unable to successfully complete its task. */
